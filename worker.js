...
/**
 * 解析图标字段，分离类名和颜色代码.
 * @param {string} iconField - 原始图标字段 (e.g., "fas fa-home #ff0000")
 * @returns {object} - { iconClass: string, iconStyle: string }
 */
function parseIconAndColor(iconField) {
    if (!iconField) return { iconClass: '', iconStyle: '' };
    // 正则表达式：匹配末尾的颜色代码（hex, rgb, rgba）
    ...
    if (match) {
        ...
        const iconColor = match[2].trim();
        return { 
            iconClass: iconClass, 
            iconStyle: `color:${iconColor};` // 预先生成样式字符串
        };
    }
    
    // 未找到颜色，只返回类名
    return { 
        iconClass: iconField.trim(), 
        iconStyle: '' 
    };
}
...
async function handleRequest({ request, env, ctx }) {
	...
	try {
		...
		// --- START: 新增 fontawesome.html 路由 ---
			...
			// --- START: MODIFICATION ---
			let categories = JSON.parse(await env.XYRJ_CONFIG.get("WidgetCategory") || "[]");
			data["widgetCategoryList"] = categories.map(cat => {
				const iconData = parseIconAndColor(cat.icon);
				return {
					...cat,
    ...
    try {
        ...
        function processMenuItems(items) {
            ...
            items.forEach(item => {
                    ...
                    item.iconClass = iconData.iconClass; // e.g., "fas fa-home"
                    item.iconStyle = iconData.iconStyle;   // e.g., "color:#ff0000;"
...
data["widgetTagList"] = Array.from(allTags).map(tag => {
	...
	// --- START: MODIFICATION ---
	data["widgetCategoryList"] = allCategories.map(cat => {
		const iconData = parseIconAndColor(cat.icon);
		return {
			...cat, // 传递 'name' 等原始属性
			iconClass: iconData.iconClass,
